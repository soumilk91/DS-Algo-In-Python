...
class Solution:
    def verticalTraversal(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        # Dictionary to hold positions as (row_index, value)
        positionsDict = defaultdict(list)
        queue = deque([(root, 0, 0)])  # (node, column index, row index)

        while queue:
            node, position, row = queue.popleft()
            positionsDict[position].append((row, node.val))

            # Enqueue left and right children with updated positions
            if node.left:
                queue.append((node.left, position - 1, row + 1))
            if node.right:
                queue.append((node.right, position + 1, row + 1))

        # Sorting the column indices and their corresponding node values
        sortedKeys = sorted(positionsDict.keys())
        for key in sortedKeys:
            # Sort by row first, then by value
            result.append(sorted(sorted(positionsDict[key])))

        # Extract only the values from the sorted tuples
        return [[val for row, val in col] for col in result]