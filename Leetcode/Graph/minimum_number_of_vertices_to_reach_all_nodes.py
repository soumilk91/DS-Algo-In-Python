...
class Solution:
    def findSmallestSetOfVertices(self, n: int, edges: List[List[int]]) -> List[int]:
        # If there are no incoming edges towards the node, that node should be considered
        # Calculate incoming and outgoing vertices from the given data

        incomingDegree = [0] * n
        outgoingDegree = [0] * n
        for edge in edges:
            outgoingDegree[(edge[0])] += 1
            incomingDegree[(edge[1])] += 1
        # print(incomingDegree)
        # print(outgoingDegree)
        results = []
        for index, degree in enumerate(incomingDegree):
            if degree == 0:
