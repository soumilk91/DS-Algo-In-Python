...
should only be printed at most every 10 seconds (i.e. a message printed at timestamp t will prevent other identical
...
class Logger:
    ...
    def __init__(self):
        self.msg_to_next_allowed_timestamp = {}

    def shouldPrintMessage(self, timestamp: int, message: str) -> bool:
        if message not in self.msg_to_next_allowed_timestamp:
            self.msg_to_next_allowed_timestamp[message] = timestamp + 10
            return True
        else:
            if self.msg_to_next_allowed_timestamp[message] > timestamp:
                return False
            else:
                self.msg_to_next_allowed_timestamp[message] = timestamp + 10
                return True
