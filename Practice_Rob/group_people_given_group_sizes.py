...
class Solution:
    def groupThePeople(self, groupSizes: List[int]) -> List[List[int]]:
        ...
        for index, value in enumerate(groupSizes):
            if value in group_size_dict:
                if len(group_size_dict[value][-1]) < value:
                    group_size_dict[value][-1].append(index)
                else:
                    group_size_dict[value].append([index])
            else:
                group_size_dict[value] = [[index]]
        result = []
        for i in group_size_dict:
            for j in group_size_dict[i]:
                result.append(j)
        return result
